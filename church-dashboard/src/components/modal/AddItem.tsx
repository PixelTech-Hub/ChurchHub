...
const AddItem = ({ children, title, subtitle, handleSubmit, isLoading, showModal, setShowModal }: Props) => {
	// const [showModal, setShowModal] = useState(false);

	return (
		<>

			<button className="px-2 text-white lg:text-base text-[12px] bg-[#200e32] rounded-sm  font-bold lg:p-2 p-2" type="button" onClick={() => setShowModal(true)}>
				{title}
			</button>
			{showModal ? (
				<div className="bg-black bg-opacity-80 backdrop-blur-sm justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
					<div className="relative   lg:my-10 -my-52 lg:mx-24 mx-10 max-w-3xl  lg:mt-[0px] md:mt-[0px]  mt-[70px]">
						{/*content*/}
