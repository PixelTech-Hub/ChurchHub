...
interface SystemAdminTableProps {
	paginatedSystemAdmins: Admin[];
	filteredSystemAdmins: Admin[];
	loading: boolean;
	totalPages: number;
	currentPage: number;
...
const SystemAdminTable: FC<SystemAdminTableProps> = ({ currentPage, filteredSystemAdmins, loading, paginatedSystemAdmins, setCurrentPage, totalPages }) => {
	...
	if (filteredSystemAdmins.length === 0) {
		return (
			<div className="text-center py-4">
				<p className="text-red-500 dark:text-gray-400">No System Admin Found</p>
			</div>
		);
	}
		...
		<>
			<Table className="min-w-full divide-y divide-gray-200 dark:divide-gray-600">
				...
				<Table.Body className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
					{paginatedSystemAdmins.map(admin => (
